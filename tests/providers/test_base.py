...
class TestProviderMetadata:
    ...
    def test_facebook_scopes_use_safe_login_permissions_by_default(self):
        p = get_provider("facebook")
        assert p.required_scopes == ["public_profile"]
        assert "pages_show_list" not in p.required_scopes
        assert "pages_manage_posts" not in p.required_scopes
        assert "pages_read_engagement" not in p.required_scopes
